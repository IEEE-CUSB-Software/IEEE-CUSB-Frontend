      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        >
          <div className="text-center mb-8">
            ...
            >
              Check out our open vacancies and apply to become part of IEEE
              CUSB!
            </p>
          </div>
          ...
          {!isAuthenticated ? (
            <div
              className={`text-center p-8 rounded-xl border ${isDark ? 'border-gray-700 bg-gray-900/50' : 'border-gray-200 bg-gray-50'}`}
            >
              <h3
              ...
              >
                You must be logged in to view your applications and apply for
                open vacancies.
              </p>
              <Button
            ...
            >
              ...
              <p
                className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}
              >
                Your current role as {user?.role?.name} restricts you from
                applying to open vacancies.
              </p>
            </div>
          ) : isLoadingVacancies ? (
        ...
        {isAuthenticated && (
          ...
          >
            ...
            {isLoadingMyApps ? (
              ...
              <div
                className={`text-center py-8 rounded-xl border border-dashed ${isDark ? 'border-gray-700' : 'border-gray-200'}`}
              >
                <p
                  className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}
                >
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <h4
                            className={`font-semibold text-sm pr-6 ${isDark ? 'text-white' : 'text-gray-900'}`}
                          >
                            {vacancy?.title || 'Unknown Role'}
                          </h4>
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wider ${getStatusColor(app.status)}`}
                          >
                        ...
                        <p
                          className={`text-xs mb-3 line-clamp-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
                        >
                          <span className="font-medium">Why join:</span>{' '}
                          {app.extra_data.why_join}
                        </p>
